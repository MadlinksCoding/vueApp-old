<!-- =====================================================================================
FILE: src/pages/Demo10AlignCenter.vue
Goal: All columns CENTER-aligned (via alignDefault="center"). Manual "Load more" always visible.
====================================================================================== -->
<script setup>
...
const rowsOf=(n,start=1)=>Array.from({length:n},(_,i)=>({
  id:start+i,
  order:`C${(start+i).toString().padStart(4,'0')}`,
...
/* No per-column align; alignDefault will center-align everything (including headers) */
const columns = [
...
function loadMore(){
  ...
  setTimeout(()=>{
    ...
    state.loading=false
    console.log('[Demo10] appended', state.rows.length, 'hasMore:', state.hasMore)
  },250)
}
</script>
...
<template>
  <main class="max-w-[1100px] mx-auto p-6 space-y-4">
    ...
    <h2 class="text-xl font-semibold">10) Alignment — ALL CENTER (headers + cells)</h2>
