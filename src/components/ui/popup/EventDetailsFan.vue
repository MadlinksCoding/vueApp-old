    ...
    <div class="w-full h-full overflow-auto bg-white inline-flex flex-col justify-start items-start" data-test="event-details-fan" :style="{ '--event-color': eventColor }">
      <div class="self-stretch relative bg-black/25 backdrop-blur-[5px] flex flex-col justify-start items-start">
        <div class="self-stretch px-4 pt-12 pb-2 min-h-[18.75rem] relative bg-gradient-to-b from-amber-400/5 to-amber-400/30 flex flex-col justify-end items-start gap-4" data-test="event-details-fan-hero">
          ...
          <div v-if="showMenu" class="right-4 top-[16px] absolute z-20 inline-flex items-start gap-1">
      ...
      <div v-if="pendingPriceAdjustment" class="self-stretch shadow-[0px_0px_8px_0px_rgba(0,0,0,0.25)] border-b-[0.50px] border-gray-200 inline-flex justify-start items-start" data-test="event-details-fan-price-adjustment">
      ...
      <div class="self-stretch flex-1 bg-gray-50 inline-flex justify-start items-start">
        ...
        <div class="flex-1 min-w-0 self-stretch p-4 inline-flex flex-col justify-between items-start gap-2">
          ...
          <div v-if="canJoinCall" class="self-stretch inline-flex justify-start items-start gap-2.5">
...
const emit = defineEmits(['update:modelValue', 'close', 'join-call', 'open-chat', 'cancel-booking', 'accept-adjustment', 'decline-adjustment']);
...
const raw = computed(() => props.booking || props.event?.raw || {});
...
const eventColor = computed(() => pendingPriceAdjustment.value ? '#FACC15' : storedEventColor.value);
...
const creatorName = computed(() => firstText(creatorProfile.value?.displayName, creatorProfile.value?.display_name, raw.value?.creatorDisplayName, raw.value?.creatorName, mergedEvent.value?.creatorDisplayName, mergedEvent.value?.creatorName));
const creatorAvatar = computed(() => firstText(creatorProfile.value?.avatar, creatorProfile.value?.avatarUrl, creatorProfile.value?.avatar_url, raw.value?.creatorAvatar, mergedEvent.value?.creatorAvatar, mergedEvent.value?.creatorAvatarUrl));
const creatorVerified = computed(() => [
...
const chatPayload = computed(() => raw.value?.meta?.chatId ? { chatId: raw.value.meta.chatId } : (creatorId.value ? { userId: String(creatorId.value) } : {}));
const canOpenChat = computed(() => Boolean(chatPayload.value.chatId || chatPayload.value.userId));
const joinState = computed(() => getCalendarEventJoinState(props.event, { viewerRole: 'fan', now: now.value }));
...
const showMenu = computed(() => Boolean(bookingId.value) && !isEnded.value && !pendingPriceAdjustment.value);

function handleModelUpdate(value) { emit('update:modelValue', value); if (!value) emit('close'); }
function closePanel() { menuOpen.value = false; emit('update:modelValue', false); emit('close'); }
function handleJoin() { const fresh = getCalendarEventJoinState(props.event, { viewerRole: 'fan', now: new Date() }); if (fresh.canJoin && fresh.joinUrl) emit('join-call', { bookingId: bookingId.value, eventId: raw.value?.eventId, joinUrl: fresh.joinUrl, event: props.event }); }
...
function handleDocumentClick() { menuOpen.value = false; }

watch(showMenu, (visible) => { if (!visible) menuOpen.value = false; });
watch(bookingId, () => { menuOpen.value = false; });
watch(creatorId, async (id) => {
  if (profileController) profileController.abort();
  creatorProfile.value = null;
  if (!id) return;
  profileController = new AbortController();
  try {
    const response = await fetch(`${buildWpApiUrl('/users/get-profile-data')}?id=${encodeURIComponent(id)}`, { headers: { Accept: 'application/json' }, signal: profileController.signal });
    if (response.ok) creatorProfile.value = (await response.json())?.user || null;
  } catch (error) { if (error?.name !== 'AbortError') creatorProfile.value = null; }
}, { immediate: true });
onMounted(() => { timerId = window.setInterval(() => { now.value = new Date(); }, 15000); document.addEventListener('click', handleDocumentClick); });
onBeforeUnmount(() => { if (timerId) window.clearInterval(timerId); if (profileController) profileController.abort(); document.removeEventListener('click', handleDocumentClick); });
</script>
