  ...
  >
    <div class="px-4 py-3 flex flex-col gap-1.5 h-full">
      ...
      <template v-else-if="!isCounterOffer">
        ...
        <!-- Action buttons -->
        <div v-if="!isExpired" class="flex items-center gap-2 mt-1">
          <button
            v-if="sessionLink"
            type="button"
...
// ── Join ──────────────────────────────────────────────────────────────────────
function handleJoin() {
  if (!sessionLink.value || isExpired.value || !isJoinable.value) return
...
const isJoinable = computed(() => {
  ...
  return diff <= 5 * 60 * 1000 && !isExpired.value
})
...
const countdownText = computed(() => {
  ...
  if (!startMs) return '—'
  const diff = startMs - now.value
  if (diff <= 0) return 'Session finished'
