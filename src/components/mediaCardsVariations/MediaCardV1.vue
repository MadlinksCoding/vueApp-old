<template>
  <div 
    :class="[
      ...
      variant === 'grid' ? 'min-h-full aspect-video' : '',
      { 'group': true } 
    ]"
  >
    
    <div 
      class="video-container relative aspect-video cursor-pointer w-full"
      :class="[
        variant === 'latest' ? 'rounded-t max-h-[9.9rem] sm:max-h-[unset] overflow-hidden' : '',
        variant === 'resume' ? 'w-[25.75rem] overflow-hidden' : '',
        variant === 'purchased' || variant === 'default' ? 'overflow-hidden' : '',
        variant === 'grid' ? 'after:content-[\'\'] after:absolute after:inset-0 after:w-full after:h-full after:rounded-none after:bg-[linear-gradient(180deg,rgba(0,0,0,0.6)_-24.72%,rgba(0,0,0,0)_41.92%,rgba(0,0,0,0.6)_108.57%)] peer hover:absolute hover:top-1/2 hover:left-1/2 hover:-translate-x-1/2 hover:-translate-y-1/2 hover:aspect-square hover:z-[20]' : ''
      ]"
    ...
    >
      ...
      ></div>
      <div 
        v-else-if="variant === 'resume' || variant === 'purchased'"
      ...
      <div class="w-full h-full bg-cover bg-center bg-no-repeat relative">
        <div class="w-full h-full flex justify-center items-center">
          ...
          <video
            v-if="media.type === 'video' || !media.type" 
            ref="videoRef"
          ...
          <img 
            v-else
            :src="media.poster || media.videoUrl"
            alt="sample-bg-image"
      ...
      <div 
        class="flex justify-between items-center w-full absolute left-0 top-0 p-1"
        :class="variant === 'grid' ? 'z-[10000]' : 'z-[20]'"
      >
        <div 
          v-if="variant === 'grid'"
          class="px-1 py-[0.0625rem] rounded bg-[rgba(24,34,48,0.50)]"
        >
          <span class="flex items-center gap-1">
            <span class="pb-[0.0625rem]">
              <img :src="mediaTypeIcon" alt="icon" class="w-4 h-4" />
            </span>
            <span v-if="media.type === 'video'" class="text-xs text-white leading-normal tracking-[0.0075rem]">{{ media.duration }}</span>
          </span>
        </div>

        <div v-else class="flex justify-center items-center gap-[0.188rem] px-1 py-0.5 rounded bg-[rgba(24,34,48,0.50)] drop-shadow-[0_0_20px_rgba(0,0,0,0.25)]">
        ...
        >
          ...
          <span class="flex justify-center items-center gap-[0.1875rem] filter drop-shadow-[0_0_.313rem_#000]">
            <img src="https://i.ibb.co.com/7tbwzFsQ/heart.webp" alt="heart" class="w-[0.75rem] brightness-100 saturate-0" />
      ...
      <div v-if="variant === 'default'" class="flex absolute left-0 bottom-0 z-[20]">
        ...
        <span class="text-[#07f468] text-xs font-medium leading-[1.125rem] flex justify-center items-center gap-2 px-[.375rem] py-[0.188rem] bg-black">Published</span>
      </div>
      ...
      <template v-if="variant === 'resume' || variant === 'purchased'">
        <div class="flex justify-center items-center absolute top-0 left-0 w-full h-full overflow-hidden z-[10]">
          <div class="flex justify-center items-center bg-black/10 rounded-full backdrop-blur-[5px] w-20 h-20">
            <img src="https://i.ibb.co.com/BVS7xsrK/video-icon.webp" alt="play" class="w-9 h-9 opacity-60 filter brightness-0 saturate-100 invert-[68%] sepia-[3%] hue-rotate-[317deg] contrast-[101%]" />
          </div>
        </div>
        <div class="absolute bottom-0 right-0 z-[20]">
          <img src="https://i.ibb.co.com/3m62Qzvz/site-logo.webp" class="flex w-16 h-16 pointer-events-none filter brightness-0 saturate-100 invert-[100%] sepia-[11%] hue-rotate-[341deg] contrast-[95%]" />
        </div>
        <div v-if="variant === 'resume'" class="absolute bottom-0 left-0 z-[20]">
          <div class="flex justify-center items-center gap-[0.188rem] px-[0.625rem] py-0.5 bg-[#ff4848] drop-shadow-[0_0_20px_rgba(0,0,0,0.25)]">
            <img src="https://i.ibb.co.com/BVS7xsrK/video-icon.webp" class="w-3 h-3 [filter:brightness(100)_saturate(0)]"/>
      ...
      <template v-if="variant === 'latest'">
        <div class="flex flex-col justify-end absolute bottom-0 left-0 w-full h-max overflow-hidden z-[20] gap-2 [background:linear-gradient(180deg,rgba(255,255,255,0)_50%,rgba(0,0,0,.80)_100%)]">
      ...
      <template v-if="variant === 'grid'">
        <div class="flex group-hover:hidden absolute left-0 bottom-0 z-[10000]">
          <span class="flex justify-center items-center px-[0.625rem] gap-1 py-1 backdrop-filter backdrop-blur-[20px] bg-[#fb0464]">
            <span class="text-white text-xs leading-[.875rem]">Subscribe or Buy</span>
          </span>
        </div>
        ...
        <div class="hidden group-hover:flex flex-col items-start gap-3 absolute left-0 bottom-0 z-[10000] pb-2 pl-2">
          ...
          <span class="text-sm font-medium flex-grow line-clamp-1 text-white">{{ media.title }}</span>
        </div>

        <div class="absolute bottom-0 right-0 z-[10000]">
          <img src="https://i.ibb.co.com/3m62Qzvz/site-logo.webp" class="flex w-16 h-16 pointer-events-none [filter:brightness(0)_saturate(100%)_invert(100%)_sepia(11%)_saturate(3489%)_hue-rotate(341deg)_brightness(93%)_contrast(95%)]" />
        </div>
      </template>
    ...
    <div v-if="variant === 'default' || variant === 'purchased'" class="flex flex-col gap-1 w-full">
      <div class="flex items-start gap-1">
        <span class="text-sm font-medium flex-grow line-clamp-2 text-[#0c111d]" :class="variant === 'default' ? 'h-10' : 'line-clamp-1'">
          {{ media.title }}
        </span>
        <div class="relative cursor-pointer" ref="dropdownRef">
          <div class="media-select cursor-pointer rounded-[0.5rem] border-none" @click.stop="toggleDropdown">
            <div class="dash-select__trigger">
              <span class="cursor-pointer w-6 h-6 rounded-[0.313rem] flex justify-center items-center hover:bg-[rgba(41,112,255,0.2)]">
                <img src="https://i.ibb.co.com/bgGv5xSL/3-dots.webp" class="w-4 h-4" />
              </span>
            </div>
            <transition enter-active-class="transition-all duration-200 ease-out origin-top" enter-from-class="scale-95 opacity-0 h-0" enter-to-class="scale-100 opacity-100 h-auto" leave-active-class="transition-all duration-200 ease-out origin-top" leave-from-class="scale-100 opacity-100 h-auto" leave-to-class="scale-95 opacity-0 h-0">
              <div v-if="isDropdownOpen" class="dash-options-container w-full absolute z-[30] right-[8px] w-max overflow-hidden shadow-lg rounded-[0.625rem]" :class="dropdownPositionClass">
                <div class="border border-[rgba(186,188,203,0.5)] rounded-[0.625rem] bg-white overflow-hidden py-1 md:py-0">
                  <div v-for="(option, index) in menuOptions" :key="index" class="option flex items-center justify-center gap-[0.5rem] hover:bg-[#f2f4f7] h-[4.25rem] md:h-max px-[0.625rem] md:px-2 py-[0.625rem]" :class="{'border-t border-[#dee5ec]': option.divider}" @click.stop="handleOptionClick(option.value)">
                    <div class="option-inner-container flex items-center flex-1 gap-6 md:gap-[0.75rem]">
                      ...
                      <div class="flex items-center justify-between md:justify-start gap-[0.625rem] w-full">
                        <span class="font-medium capitalize text-balance" :class="[ option.danger ? 'text-[#ff4405]' : 'text-[#0c111d]', variant === 'purchased' ? 'text-[0.875rem]' : 'text-base' ]">
                          {{ option.label }}
                        </span>
                        <img v-if="option.hasArrow" src="https://i.ibb.co.com/4bwSScz/svgviewer-png-output-51.webp" class="w-6 h-6" />
                      </div>
                    </div>
                  </div>
                </div>
    ...
    <div 
      v-if="variant === 'grid'"
      class="h-20 flex flex-col gap-1 w-full peer-hover:absolute peer-hover:bottom-[-5.5rem] md:peer-hover:bottom-[-5rem] xl:peer-hover:bottom-[-5.5rem]"
    >
      <div class="flex items-start gap-1">
        <span class="text-sm font-medium flex-grow line-clamp-1 text-[#0c111d]">
          {{ media.title }}
        </span>
        <div class="relative cursor-pointer" ref="dropdownRef">
          <div class="media-select cursor-pointer rounded-[0.5rem] border-none" @click.stop="toggleDropdown">
            ...
            <transition enter-active-class="transition-all duration-200 ease-out origin-top" enter-from-class="scale-95 opacity-0 h-0" enter-to-class="scale-100 opacity-100 h-auto" leave-active-class="transition-all duration-200 ease-out origin-top" leave-from-class="scale-100 opacity-100 h-auto" leave-to-class="scale-95 opacity-0 h-0">
              <div v-if="isDropdownOpen" class="dash-options-container absolute left-auto right-0 top-[calc(100%+0.5rem)] z-[9999] w-max overflow-hidden shadow-lg rounded-[0.625rem]" :class="dropdownPositionClass">
                <div class="border border-[rgba(186,188,203,0.5)] rounded-[0.625rem] bg-white overflow-hidden">
                  <div v-for="(option, index) in menuOptions" :key="index" class="option flex items-center justify-center gap-[0.5rem] hover:bg-[#f2f4f7] h-[4.25rem] md:h-max px-[0.625rem] md:px-2 py-[0.625rem]" :class="{'border-t border-[#dee5ec]': option.divider}" @click.stop="handleOptionClick(option.value)">
                    <div class="option-inner-container flex items-center flex-1 gap-[0.5rem]">
                      <img :src="option.icon" class="w-4 h-4 object-cover" :class="option.value === 'profile' ? 'rounded-full' : ''" />
