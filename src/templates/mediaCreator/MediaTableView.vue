<template>
    <div
        ...
        <div class="flex flex-col w-max border-t border-[#667085] xl:w-full [&>*:nth-child(odd)]:bg-[#F2F4F780] dark:[&>*:nth-child(odd)]:bg-background-dark-app/40">
            ...
            <div v-else v-for="item in items" :key="item.id" class="flex w-full">
                ...
                <div
                    ...
                    <div class="w-full flex flex-col gap-0.5">
                        <h3 class="text-xs leading-normal font-semibold line-clamp-3 text-[#0C111D] dark:text-text">
                            {{ item.title }}
                        </h3>

                        <div class="flex items-center gap-2">
                            <div v-for="(tag, index) in item.tags" :key="index"
                                class="flex justify-center items-center px-1.5 py-1 bg-white dark:bg-background-dark-app">
                                <span class="text-xs leading-normal font-medium text-[#344054] dark:text-text-dark-mutedBluish">{{ tag }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-1">
                        <div class="flex">
                            <span class="flex justify-center items-center p-1 gap-2 backdrop-blur-[10px] bg-[#07f468]">
                                <img src="https://i.ibb.co.com/bMFQRqBP/upload.webp" alt="upload" class="w-4 h-4 dark:[filter:brightness(0)_saturate(100%)_invert(100%)]" />
                            </span>
                            <span
                ...
                <div
                    ...
                    <span class="text-xs leading-normal font-semibold truncate text-[#0C111D] dark:text-text">
                        {{ item.price }}
                    </span>
                </div>
                ...
                <div
                    ...
                    <ul class="flex flex-col gap-0.5 overflow-hidden">
                        <li v-for="(tier, tIndex) in item.tiers" :key="tIndex"
                            class="pl-5 text-xs leading-normal font-semibold truncate max-w-[9.25rem] xl:max-w-[unset] text-[#0C111D] dark:text-text relative before:content-[''] before:absolute before:w-1 before:h-1 before:bg-[#0C111D] before:rounded-full before:left-2 before:top-1/2 before:-translate-y-1/2">
                            {{ tier }}
                        </li>
                    </ul>
                </div>
                ...
                <div
                    ...
                    <ul class="flex flex-col gap-0.5 overflow-hidden">
                        <li v-for="(place, pIndex) in item.appearOn" :key="pIndex"
                            class="pl-5 text-xs leading-normal font-semibold truncate max-w-[6.9375rem] xl:max-w-[unset] text-[#0C111D] dark:text-text relative before:content-[''] before:absolute before:w-1 before:h-1 before:bg-[#0C111D] before:rounded-full before:left-2 before:top-1/2 before:-translate-y-1/2">
                            {{ place }}
                        </li>
                    </ul>
                </div>
                ...
                <div
                    ...
                    <span class="text-[0.77875rem] leading-[1.125rem] text-[#475467] dark:text-text-dark-mutedBluish">{{ item.date }}</span>
                </div>
                ...
                <div
                    ...
                    <div class="flex justify-center items-center px-3 py-2">
                        <div class="w-6 h-6 flex justify-center items-center cursor-pointer">
                            <img src="https://i.ibb.co.com/BV4pbgNr/pencil-01.webp" alt="edit" class="w-4 h-4 dark:[filter:brightness(0)_saturate(100%)_invert(100%)]">
                        </div>
                    </div>

                    <div class="flex justify-center items-center px-3 py-2">
                        <div class="w-6 h-6 flex justify-center items-center cursor-pointer">
                            <img src="https://i.ibb.co.com/Q7jRP6S5/dots-vertical.webp" alt="settings" class="w-4 h-4 dark:[filter:brightness(0)_saturate(100%)_invert(100%)]">
                        </div>
                    </div>
                </div>
            </div>
