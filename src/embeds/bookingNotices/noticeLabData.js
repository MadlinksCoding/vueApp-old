...
export const LAB_NOTICE_VARIANTS = Object.freeze([
  ...
  { value: "summary-single", label: "Activity summary — one item" },
...
export function createBookingNoticeLabNotice(variant, options = {}) {
  ...
  if (variant === BOOKING_NOTICE_TYPES.SUMMARY || variant === "summary-single") {
    ...
    const isSingleSummary = variant === "summary-single";
    ...
    return {
      ...
      sections: isSingleSummary ? [{
        type: BOOKING_NOTICE_TYPES.EVENTS_TODAY,
        label: "Events today",
        totalCount: 1,
        items: [eventsTodayItems[0]],
      }] : [
        { type: BOOKING_NOTICE_TYPES.READY_TO_JOIN, label: "Ready to join", totalCount: 1, items: [ready] },
        ...
        { type: BOOKING_NOTICE_TYPES.BOOKING_REQUEST, label: "New booking requests", totalCount: requestItems.length, items: requestItems },
...
export function createAllBookingNoticeLabNotices(options = {}) {
  ...
  return LAB_NOTICE_VARIANTS.map((entry, index) => {
    const notice = createBookingNoticeLabNotice(entry.value, { ...options, sequence: (Number(options.sequence) || 1) + index });
    if (!options.mixedPositions) return notice;
    return { ...notice, config: { desktopPosition: positions[index % positions.length] } };
  });
}
